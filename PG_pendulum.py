...
if __name__ == "__main__":
	...
	parser.add_argument("--env", default="Pendulum-v1")			 # Gymnasium environment name
	...
	for e in range(args.n_iter):
		...
		if moving_avg_reward>=-50.:
			...
			if consistency_count == 10:
				...
				torch.save(learner.policy.state_dict(),f'./policy_model_{args.algo}_pendulum.ckpt')
				break
		else:
			consistency_count = 0
	torch.save(learner.policy.state_dict(),f'./policy_model_{args.algo}_pendulum.ckpt')
	...
	pickle.dump(reward_log,open(f'./reward_log_{args.algo}_pendulum.pkl','wb'))
	...
	plt.savefig(f'reward_plot_{args.algo}_{args.lr}_{args.batch_size}_{args.discount}_pendulum.png')


	# next, we render the learned agent
	# load the saved model
	# learner.policy.load_state_dict(torch.load('./policy_model_Baseline_pendulum.ckpt'))
	# learner.policy.eval()
