...
if __name__ == "__main__":
	...
	parser.add_argument("--env", default="MountainCarContinuous-v0")			 # Gymnasium environment name
	...
	for e in range(args.n_iter):
		...
		# if moving_avg_reward>=100.:
		# 	consistency_count += 1
		# 	if consistency_count == 10:
		# 		# means that our agent has achieved a cumulative reward of >=100 for the last 10 episodes
		# 		# we can now stop our agent
		# 		torch.save(learner.policy.state_dict(),f'./policy_model_{args.algo}_mount_car.ckpt')
		# 		break
		# else:
		# 	consistency_count = 0
	torch.save(learner.policy.state_dict(),f'./policy_model_{args.algo}_mount_car.ckpt')
	...
	pickle.dump(reward_log,open(f'./reward_log_{args.algo}_mount_car.pkl','wb'))
	# reward_log=pickle.load(open(f'./reward_log_{args.algo}.pkl','rb'))
	plt.subplots_adjust(bottom=0.2)
	plt.plot(reward_log)
	plt.title(f'Training curve for {args.env} environment')
	plt.xlabel('Episodes')
	...
	plt.savefig(f'reward_plot_{args.algo}_{args.lr}_{args.batch_size}_{args.discount}_mount_car.png')


	# next, we render the learned agent
	# load the saved model
	# learner.policy.load_state_dict(torch.load('./policy_model_Baseline_mount_car.ckpt'))
	# learner.policy.eval()
